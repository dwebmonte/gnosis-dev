...
const JA = {
	...
	parseQuery( query ) {
		let 
			fields = {},
			path_fields, path_cond, prev_entity, sub;
		
		sub = query.split("?");
		
		if ( sub.length == 1 ) {
			path_fields = sub[0].trim();
		} else if ( sub.length == 2 ) {
			path_fields = sub[0].trim();
			path_cond = sub[1].trim();
		} else {
			console.error("[JA.query] - больше одного знака вопроса в запросе");
			return false;
		};
		
		
		path_fields.split(",").forEach( (item, index) => {
			...
			if ( sub.length == 1 ) {
				if ( prev_entity === undefined ) {
					console.error("[JA.query] - не задана первая сущность в запросе");
					return false;
				} else {
					field = sub[0].trim();
					entity = prev_entity;
				...
				prev_entity = entity = sub[0].trim(),
				field = sub[1].trim();
			} else {
				console.error("[JA.query] - больше одной точки в полях");
				return false;
			};
			

				
			if ( fields[ entity ] === undefined ) fields[ entity ] = {};
			if ( fields[ entity ][ field ] === undefined ) fields[ entity ][ field ] = {};
			
		});		
		

		if ( path_cond !== undefined ) {
			path_cond.split("&&").forEach( (item, index) => {
								...
								if ( response.status == 200 ) {
									...
									response.data.data.forEach(item_response => 	{ 
										...
										if (item_response.type == entity.name) {
											...
											JA.store[ entity.name ].forEach(function (item_store, index_store) {
