...
const JA = {
	...
	*/
	...
	prepare( new_entites ) {
		...
		/*
		for ( let entity in JA.store ) {
			if ( entity != '_old' && entity != 'front' ) {
				env.data[ entity ] = _.map( JA.store[ entity ], _.clone);
			};
		};
		*/
				...
				handler: ( after, before ) => {
					...
					if ( data.patch.length > 0 ) {
						data.patch.forEach(function (params, index) {
							...
							axios.patch( entity.name, params ).then(response => {
	...
	ukIDByTitle( uk_title ) {
		
		let res = 0;
		
		uk_title = uk_title.trim();
		
		JA.store["uk"].forEach( (item, ind) => {
			if ( uk_title == item.title ) res = item.id;
		});
	...
	ukTitleByID( uk_id ) {
		...
		JA.store["uk"].forEach( (item, ind) => {
			if ( uk_id == item.id ) res = item.title;
		});
