"use strict";
env.pages.deploy_demo = {

props: {
	
	method: { type: String },
	entity: { type: String },
	view: { type: String, default: "card" },

},	


// ~~~~~~~~~~~~~~~~~~~~~~				data
data: function () {
	return {
		web3: undefined,
	
		eth_key: '1TSEZZWRAPE332SGK4TPU7NYDSX71W2AD4',
		address1: '0x84e30B7A76e88348f044F8844017011BceB902a6',
		address1_pk: 'dcebe6f966bbab5f1c1b14a09815a388aa39cf80c72794f3556015a7926ad8e5',
		data: '',
		//value_send: 0.01,
		txHash: '',
		
		gasLimit: 1000000,
		gasPrice: 10,
		
		
		confirmDeploy: {}

	}
},	

methods: {
	...env.methods,
	
	sendTransaction() {
	
		web3 = Eth.handlerWeb3();

		const account1 = this.address1 // Your account address 1

		const privateKey1 = Buffer.from( this.address1_pk , 'hex')

		web3.eth.getTransactionCount(account1, (err, txCount) => {
			
			if ( "web3.eth.getTransactionCount", err ) { console.error( err ); env.pnotify.danger(`Error ${err.code}`, err.message) };

			console.log("txCount=" + txCount);  
			
		  
		  const data = this.data;


			const txObject = {
				nonce:    web3.utils.toHex(txCount),
				gasLimit: web3.utils.toHex(`${this.gasLimit}`), 
				gasPrice: web3.utils.toHex(web3.utils.toWei(`${this.gasPrice}`, 'gwei')),
				data: data
			}

			const tx = new Tx( txObject, {'chain':'rinkeby'} )
			tx.sign(privateKey1)

			const serializedTx = tx.serialize()
			const raw = '0x' + serializedTx.toString('hex')

			web3.eth.sendSignedTransaction(raw, (err, txHash) => {
				if ( !err ) {
					env.pnotify.success("Deploy is success!", `txHash is ${txHash}`);
					setTimeout(() => { this.txHash = txHash; }, 2000);
				} else { console.error( err ); env.pnotify.danger(`Error ${err.code}`, err.message) };
				
			})
			.on("connected", function(subscriptionId){
				console.info( 'connected', subscriptionId );
			})
			.on('data', event => {
				console.info( 'data', event );
			})
			.on('changed', event => {
				console.info( 'changed', event );
			})			  
			.on('transactionHash', hash => {
				console.info( 'transactionHash', hash );
			})
			.on('receipt', receipt => {
				console.info( 'receipt', hash );
			})
			.once('confirmation', (confirmationNumber, receipt) => {
				console.log( 'confirmation' );
				receipt.confirmationNumber = confirmationNumber;
				this.confirmDeploy = receipt;
			})


		  
		})


		
		
	
	}
},
	
	
	
computed: {
	...env.computed,	
	


},


created() {

},



template: `

<div class="content">
	<div class="row">
		
		<div class="card" style="width: 100%">
			<div class="card-header  bg-slate-600 header-elements-inline">
				<h6 class="card-title">Deploy Contract</h6>
				<!--
				<div class="header-elements" >
					<button type="button" @click=" $router.push({path: '/variables-new' }) " class="btn btn-success"><i class="icon-plus2 mr-2"></i> Добавить</button>
				</div>
				-->
			</div>
			
			<div class="card-body">
			
				<form @submit.prevent=" sendTransaction() ">
					<div class="form-group row">
						<label class="col-form-label col-lg-2">Wallet address sender</label>
						<div class="col-lg-10">
							<input type="text" required class="form-control form-control-lg" v-model="address1 ">
						</div>
					</div>						
					<div class="form-group row">
						<label class="col-form-label col-lg-2">Private key sender</label>
						<div class="col-lg-10">
							<input type="text" required class="form-control form-control-lg" v-model="address1_pk ">
						</div>
					</div>
					
					<div class="form-group row">
						<label class="col-form-label col-lg-2">Data</label>
						<div class="col-lg-10">
							<input type="text" required class="form-control form-control-lg" v-model="data ">
						</div>
					</div>
					<div class="form-group row">
						<label class="col-form-label col-lg-2">Gas Limit</label>
						<div class="col-lg-10">
							<input type="text" required class="form-control form-control-lg" v-model="gasLimit">
						</div>
					</div>	
					<div class="form-group row">
						<label class="col-form-label col-lg-2">Gas price</label>
						<div class="col-lg-10">
							<input type="text" required class="form-control form-control-lg" v-model="gasPrice">
						</div>
					</div>						
					<div class="form-group row">
						<div class="col">
							<div class="text-right">
								<button class="btn btn-success legitRipple">Deploy Contract <i class="icon-paperplane ml-2"></i></button>
							</div>							
						</div>								
					</div>						
				</form>


				<!-- Confirmation Info -->

				<fieldset v-if=" confirmDeploy.blockHash">
					<legend class="text-uppercase font-size-sm font-weight-bold">Confirmation Info</legend>									
					
					<table class="table table-striped table-bordered mt-2">
						<tbody>	

							<tr v-if=" confirmDeploy.confirmationNumber">
								<td><label class="font-weight-semibold"><code>confirmationNumber</code></label></td>
								<td>{{confirmDeploy.confirmationNumber}}</td>
							</tr>
							
							<tr v-if=" confirmDeploy.blockHash">
								<td><label class="font-weight-semibold"><code>blockHash</code></label></td>
								<td><a :href=" 'https://rinkeby.etherscan.io/tx/' + confirmDeploy.blockHash " target="_blank">{{confirmDeploy.blockHash}}</a></td>
							</tr>							
						
							<tr v-if=" confirmDeploy.blockNumber">
								<td><label class="font-weight-semibold"><code>blockNumber</code></label></td>
								<td>{{confirmDeploy.blockNumber}}</td>
							</tr>						
							
							<tr v-if=" confirmDeploy.contractAddress">
								<td><label class="font-weight-semibold"><code>contractAddress</code></label></td>
								<td><a :href=" 'https://rinkeby.etherscan.io/address/' + confirmDeploy.contractAddress " target="_blank">{{confirmDeploy.contractAddress}}</a></td>
							</tr>

							<tr v-if=" confirmDeploy.cumulativeGasUsed">
								<td><label class="font-weight-semibold"><code>cumulativeGasUsed</code></label></td>
								<td>{{confirmDeploy.cumulativeGasUsed}}</td>
							</tr>	
							
							<tr v-if=" confirmDeploy.from">
								<td><label class="font-weight-semibold"><code>from</code></label></td>
								<td><a :href=" 'https://rinkeby.etherscan.io/address/' + confirmDeploy.from " target="_blank">{{confirmDeploy.from}}</a></td>
							</tr>	

							<tr v-if=" confirmDeploy.gasUsed">
								<td><label class="font-weight-semibold"><code>gasUsed</code></label></td>
								<td>{{confirmDeploy.gasUsed}}</td>
							</tr>	

							<tr v-if=" confirmDeploy.transactionHash">
								<td><label class="font-weight-semibold"><code>transactionHash</code></label></td>
								<td><a :href=" 'https://rinkeby.etherscan.io/tx/' + confirmDeploy.transactionHash " target="_blank">{{confirmDeploy.transactionHash}}</a></td>
							</tr>							
							
							<tr v-if=" confirmDeploy.transactionIndex">
								<td><label class="font-weight-semibold"><code>transactionIndex</code></label></td>
								<td>{{confirmDeploy.transactionIndex}}</td>
							</tr>	
							
						</tbody>		
					</table>

					
				</fieldset>



				<fieldset class="mt-4">
					<legend class="text-uppercase font-size-sm font-weight-bold">Transaction Info</legend>									
					<web3-transaction-table :txHash="txHash"></web3-transaction-table>					
				</fieldset>

			</div>
		</div>	
	
	
	</div>
</div>


`
}